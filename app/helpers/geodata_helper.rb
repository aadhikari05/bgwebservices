module GeodataHelper
        ...
        def GeodataHelper.get_state_business_type_geodata_sites(this_result, state_id, business_type_id)
            #Add State Results
        ...
        #########################################################
        def GeodataHelper.getBusinessTypeIdFromBusinessType(business_type)
            GeodataSubcategory.find(:all, :select => "id", :conditions => ["isExclusive=1 and isActive=1 and name = ?",business_type])
        end

        def GeodataHelper.getFeatureAndStatebyZip(zip)
        ...
        def GeodataHelper.CountySpecsByNameQuery(feature_name, state_id)
        ...
        def  GeodataHelper.PermitMeSitesByFeatureIdQuery(feature_id)
            GeodataSite.find(:all, :select => "url,name, feature_id", :conditions => ["feature_id = ? and url is not null",feature_id])
        end

        def  GeodataHelper.PermitMeFeatureAltNameMappingQuery(alternate_name)
        ...
        def GeodataHelper.permitMeCountySpecsByNameQuery(feature_id)
            GeodataSite.find(:all, :select => "id,description, url,name, feature_id", :conditions => ["feature_id = ? and url is not null", feature_id])
        end
        ...
        ####################################################
        def  GeodataHelper.PermitMeFeatureMappingQuery(feature_name, alternate_name)
        ...
        def  GeodataHelper.PermitMeFeatureWithStateMappingQuery(feature_name, alternate_name, state_id)
        ...
        def GeodataHelper.PermitMeResultsByStateQuery(state_id)
            strQuery = "select s.name as subcategory,rg.description, p.url, p.link_title "
        		strQuery += "from geodata_resource_groups rg join geodata_resources p on p.geodata_resource_group_id = rg.id "
        		strQuery += "left join geodata_categories c on rg.geodata_category_id <=> c.id "
        		strQuery += "left join geodata_subcategories s on rg.geodata_subcategory_id <=> s.id "
        		strQuery += "left join geodata_sections sec on rg.geodata_section_id <=> sec.id "
        		...
        		strQuery += "order by geodata_category_id, geodata_subcategory_id, geodata_section_id"
        		GeodataResourceGroup.find_by_sql([strQuery,state_id])
        end

        def GeodataHelper.PermitMeResultsByBusinessTypeQuery(state_id, business_type_id)
            strQuery = "select s.name as subcategory, p.url, p.link_title "
        		strQuery += "from geodata_resource_groups rg join geodata_resources p on p.geodata_resource_group_id = rg.id "
        		strQuery += "left join geodata_categories c on rg.geodata_category_id <=> c.id "
        		strQuery += "join geodata_subcategories s on rg.geodata_subcategory_id <=> s.id "
        		strQuery += "left join geodata_sections sec on rg.geodata_section_id <=> sec.id "
        		strQuery += "where state_id= ? and url is not null "
        		strQuery += "and(s.id = ?) "
        		strQuery += "and(s.isActive = 1 or s.isActive is null) "
        		strQuery += "order by geodata_category_id, geodata_subcategory_id, geodata_section_id"
        		GeodataResourceGroup.find_by_sql([strQuery,state_id,business_type_id])
        end
      ...
      ####################################################
      def GeodataHelper.process_rules(county_array)
      ...
      def GeodataHelper.prune_array(this_array, fips_compare_array)
      ...
      def GeodataHelper.prune_unincorporated_areas(this_array)
      ...
      def GeodataHelper.combine_multiple_urls(this_array)
      ...
      def GeodataHelper.prune_vt_h1_6_counties(this_array)
          #If state is vermont, then drop any h1-h6 counties
          if @state_alpha.eql?("vt")
              fips_compare_array = ["h1","h2","h3","h4","h5","h6"]
              this_array = GeodataHelper.prune_array(this_array, fips_compare_array)
          end
          
          this_array
      end
      
      def GeodataHelper.prune_h4_h6_counties(this_array)
